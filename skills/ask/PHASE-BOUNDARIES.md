...
| **Fresh session** | Empty the context window and start from nothing (the DSH equivalent of `/clear`). |
| **`/handoff`** | Write a handoff file + mnemon hot-memory entries and seed a session anywhere with them. |
| **Subagent** | Send the task to its own context window (`subagent` tool) and get a report back. |
| **Compact** | Run `/handoff` to mnemon memory, then a fresh session reads it (the DSH equivalent of `/compact`). |
...
**2. Is the context irrelevant to what comes next?** Is everything in this session (the exploration, the decisions, the dead ends) disposable? If so, start a **fresh session** (the DSH equivalent of `/clear`). It is the cheapest move on the board: it takes no time and hands back the whole window. It also isn't terminal: the old session stays resumable.
...
**5. Otherwise, compact.** Relevant context, same harness, same directory, and you need to stay in the loop: this is where the tree lands, and it lands here often. Run `/handoff` (it persists the essentials to mnemon memory, which every future turn sees), then open a fresh session and state the next phase - e.g. "we're going to QA this area" - so the handoff keeps what the next phase needs.

Compacting is the **default, not the first reach**. It sits at the bottom because the four questions above it are all cheaper or more precise. The failure mode when people start here is a fresh session that is confidently wrong about a decision the summary flattened.
