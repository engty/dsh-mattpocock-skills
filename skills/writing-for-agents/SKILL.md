...
description: Writing documents for agents. Use when creating or editing skills, or modifying AGENTS.md or CLAUDE.md. 中文触发：写 skill、写 AGENTS.md、写 agent 文档。
---
...
In DSH the environment differs from Claude Code in ways that change what to cache:

- Both AGENTS.md and CLAUDE.md are loaded. Project root = nearest ancestor with .git, else cwd; the user-global $DSH_HOME/AGENTS.md is loaded too. A doc restating either is a cache that can go stale.
- No slash commands (/clear, /compact), no Task tool, no hooks, no claude CLI. Equivalents: a new session ≈ /clear; the handoff skill or mnemon memory ≈ /compact; fan-out → subagent / subagent_fork; asking the user → ask_user_question or a GenUI form.
- Mnemon runtime memory is a new tier between always-loaded context and on-demand files: a few hot one-liners injected every turn. Use it for pointers and preferences, never for content that already lives in a file. Memory, files, and documents must not duplicate.
- A user-invoked skill (disable-model-invocation: true) is absent from the model's skill catalog: only the human typing /name can fire it, and no other skill can. DSH also supports user-invocable natively.
